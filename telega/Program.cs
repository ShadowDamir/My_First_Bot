...
namespace telega
{
    class Program
    {
        private string connectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\damir\source\repos\TelegramBot\telega\Database1.mdf;Integrated Security=True";
        private static Program _program; //задание локальной переменной для данной программы
        private readonly TelegramBotClient _bot = new TelegramBotClient(" Place for bot token "); //Создание клиента бота с данным токеном
            ...
            try
            {
                ...
                switch (message.Text.ToLower())
                {
                    ...
                    case "текущая температура воздуха мск":
                        ...
                        WebRequest request = WebRequest.Create("http://api.openweathermap.org/data/2.5/weather?q=Moscow&APPID= Place For Token ");
                        request.Method = "POST";
